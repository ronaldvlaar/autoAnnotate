...
def fill_gaps(ann, vidstats):
    ...
    for idx, row in ann.iterrows():
        ...
        duration = float(vidstats[vidstats['file'] == row['file']]['frames'])/ float(vidstats[vidstats['file'] == row['file']]['rate'])
        
        #case first annotation not started at 0
        if row['begin'] != 0.0:
            newrow = [row['tier'], 'not_used', 0.0, row['beginmm'], row['begin'], 'not_used', row['begin'], row['class'], row['file'], row['case']]
        ...
        #case last part not annotated
            ...
            newrow = [row['tier'], row['endmm'], row['end'], 'not_used', duration, 'not_used', duration-row['end'], row['class'], row['file'], row['case']]
            df.loc[len(df)] = newrow
...
def get_frame_level_data(ann, files=None):
    ...
    for f in files:
        ...
        startframes = [(int(b*stat['rate']), c) for b, c in begins]
...
annotations_frame = get_frame_level_data(annotations, files=annotations_double['file'].unique())
annotations_frame.to_csv('annotations_frame.csv')
annotations_double_frame = get_frame_level_data(annotations_double, files=annotations_double['file'].unique())
